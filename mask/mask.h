...
/*自分用メモ
  最初に数字をセットする仮定においてsetNumberListで一気にセットするのではなく、SetColumnAtRandomで一行ずつセットするほうがよさそう
  探索の袋工事に入った時のために、リセットして最初から探索をやり直す関数も用意する必要がありそう。
  数字とかアルファベットに何らかの操作をしたら、setMaskWithNumで明示的にmask本体を更新しなければならない。
  どうやらcheckColumnでちゃんと動作してないみたい。明日はそのデバック」を使用。今日はよっぱらってるからっこでおわり。おやsづいみせかい。
