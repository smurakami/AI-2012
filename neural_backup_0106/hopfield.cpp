...
FLOAT Energy()
{
  ...
  for (i=1;i<=X;i++)
    for (j=1;j<=Y;j++)
      for (k=1;k<=Z;k++)
        tm1 += term11(i,j,k)*V(i,j,k) + term12(i,j,k)*V(i,j,k)+ term13(i,j,k)*V(i,j,k);

  for (i=1;i<=X;i++){
    for (j=1;j<=Y;j++) {
      for (j=1;j<=Y;j++) {
        tm2 += term2(i,j,k)*V(i,j,k);
      }
    }
  }
...
float U(int i, int j, int k)
{
  ...
  sum = Aval*(term11(i,j,k)+term12(i,j,k)+term13(i,j,k)) + Bval * term2(i, j, k) + Cval*term4(i, j, k);

  return -sum;

}

double output_f(double x)
{
  if(Rand01() < sigmoid(x / temperature - 0.5)) return 1.0;
  else return 0.0;
}
...
BOOL PropagateUnit(NET* Net, INT i)
{
  ...
  to_dx(i,&idx,&jdx,&kdx);
  Out = Uvalue = U(idx,jdx,kdx);

  //シグモイド関数を使って計算してみる
  Out = output_f(Uvalue);//sigmoid((Uvalue - 5) / temperature);
...
void PropagateNet(NET* Net)
{
  ...
  if(Iteration-IterationOfLastChange < 10*Net->Units){
    //現状の表示
	  fprintf(f, "E(%d) = %f \n",Iteration,Energy());
	  fprintf(f, "temperature(%d) = %f \n",Iteration,temperature);
