  ...
  for(i = 0; i < SIZE; i++){
    for(j = 0; j < SIZE; j++){
      if(AI->neu[i][j].state & b0000_0001){//もしも興奮していたら
        unsigned char from = AI->neu[i][j].from;
        AI->neu[i][j].from = AI->neu[i][j].from & b0000_1111;//from情報をリセット
        if(j < SIZE - 1){//rightにシナプスがのびていたら
          if((AI->neu[i][j].state & RIGHT) &&
             !(from & RIGHT)){
            AI->neu[i][j+1].state = AI->neu[i][j+1].state | NEIB;//次に興奮するという情報
            AI->neu[i][j+1].from = AI->neu[i][j+1].from | LEFT;//どこから信号が伝わったか記録。連続発火防止
          }
        }
        if(i > 1){//upにシナプスがのびていたら
          if((AI->neu[i-1][j].state & DOWN) &&
             !(from & UP)){
            AI->neu[i-1][j].state = AI->neu[i-1][j].state | NEIB; 
            AI->neu[i-1][j].from = AI->neu[i-1][j].from | DOWN; 
          }
        }
        if(j > 1){//leftにシナプスがのびていたら
          if(AI->neu[i][j-1].state & RIGHT &&
             !(from & RIGHT)){
            AI->neu[i][j-1].state = AI->neu[i][j-1].state | NEIB; 
            AI->neu[i][j-1].from = AI->neu[i][j-1].from | RIGHT; 
          }
        }
        if(i < SIZE - 1){//downにシナプスがのびていたら
          if((AI->neu[i][j].state & DOWN) &&
             !(from & DOWN)){
            AI->neu[i+1][j].state = AI->neu[i+1][j].state | NEIB; 
            AI->neu[i+1][j].from = AI->neu[i+1][j].from | UP; 
          }
        }
      }
    }
  ...
  for(i = 0; i < SIZE; i++){
    for(j = 0; j < SIZE; j++){
      if(AI->neu[i][j].state & b0000_0010){//興奮しているご近所さんとつながってたら
        ...
        AI->neu[i][j].state = AI->neu[i][j].state | RETINA;//網膜からたどりつけることをメモ
