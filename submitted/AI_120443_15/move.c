...
int move(state_t st, point_t p, direction_t d){
  ...
  if(st->attack == 0){
    ...
    st->bd[p->x][p->y] = NONE;
    st->bd[p->x + dx][p->y + dy] = NONE;
    st->bd[p->x + dx + dx][p->y + dy + dy] = st->player;
    /*さらにとれる場所が無いかチェック*/
    struct point p2;
    p2.x = p->x+dx+dx;
    p2.y = p->y+dy+dy;
    st->attack = 2;//二度目以降のアタックのサイン
    st->attackingPiece->p.x = p2.x;
    st->attackingPiece->p.y = p2.y;
    st->attackingPiece->left = 0;;
    st->attackingPiece->right = 0;
    
    if(checkPieceToAttack(st, p2)){
      char buffer[30];
      st->pieceToMove = st->attackingPiece;
      p->x = p2.x;
      p->y = p2.y;
      while(1){
        printBoard(st);
        printf("you have to attack one more\n");
